...
def calc_k_one_arg(f, arg_i: int, x: float, y: list, h: float) -> list: # classic 4th rang
    k = []
    k.append(f(x, y))
    k.append(f(x, y + [h / 2 * k[0] for i in y]))
    k.append(f(x, y + [h / 2 * k[1] for i in y]))
    k.append(f(x, y + [h * k[2] for i in y]))
    return k

def calc_k(F: list, x: float, y: list, h: float) -> list[list]:
    k = []
    for i in range(len(y)):
        k.append(calc_k_one_arg(F[i], i, x, y, h))
    return k

def runge_kutta_step(F: list, x: float, y: list, h: float):
    k = calc_k(F, x, y, h)
    y_next = []
...
def expl_runge_kutta(F: list, t_0: float, y_0: list, t_final: float, grid_size: int):
    h = (t_final - t_0) / grid_size
    ...
    while (t_0 < t_final):
        ...
        y_next = runge_kutta_step(F, t_0, y, h)
        t_0 += h
        y_list.append(y_next)

    return y_list

def adams(F: list, y_0: list, t_final: float, grid_size: int):
    h = t_final / grid_size
    y_list = [y_0]
    ...
    while (t < t_final):
        ...
        y_next = [0, 0, 0]
        for i in range(len(y)):
            y_next[i] = y[i] + h * (3 / 2 * F[i](t, y) - 1 / 2 * F[i](t, y_prev))
    
        y_list.append(y_next)
        t += h
    return y_list

def backward_diff(F: list, y_0: list, t_final: float, grid_size: int):
    h = t_final / grid_size
    y_list = [y_0]
...
def explicit_methods():
    grid_size = 100000
    F = [f_x, f_y, f_z]
    y_list = expl_runge_kutta(F, 0, [1, 1, 1], 50, grid_size)
    y_list1 = adams(F, [1, 1, 1], 50, grid_size)
    y_list2 = backward_diff(F, [1, 1, 1], 50, grid_size)
...
def impl_runge_kutta(f, y_0: list, t_final: float, grid_size: int):
    h = t_final / grid_size
...
def rosenbrock(f, y_0: list, t_final: float, grid_size: int):
    h = t_final / grid_size
    y_h = h
...
def nordsik(f, y_0: list, t_final: float, grid_size: int):
    h = t_final / grid_size
    ord = 4
...
def implicit_methods():
    grid_size = 100000
    y_list, t_list = rosenbrock(F, np.array([tetta0, fi0]), 10, grid_size)
    tetta1_, fi1_ = prepare_arrays_impl(y_list)

    y_list, t_list = impl_runge_kutta(F, np.array([tetta0, fi0]), 10, grid_size)
    tetta0_, fi0_ = prepare_arrays_impl(y_list)

    y_list, t_list = nordsik(F, np.array([tetta0, fi0]), 10, grid_size)
